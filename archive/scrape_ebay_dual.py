...
async def scrape_card(unique_id, query, tier, session):
...
async def run_dual_scraper():
    ...
    async with async_session() as session:
        tasks = []
        for unique_id, query, tier in cards:
            tasks.append(run_card_with_semaphore(unique_id, query, tier, session, semaphore))
        await asyncio.gather(*tasks)

async def run_card_with_semaphore(unique_id, query, tier, session, semaphore):
    async with semaphore:
        await scrape_card(unique_id, query, tier, session)
